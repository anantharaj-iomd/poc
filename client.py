...
Usage:
    python client.py --base https://nodenorthstar.com --output ./out [--endpoint https://nodenorthstar.com/.well-known/mcp] [--deep-discovery] [--buy-again MOCK-12345] [--fetch-products]
...
MOCK_ORDER_HISTORY = {
    "MOCK-12345": {
        ...
        "status": "delivered",
        "items": [
            {"product_variant_id": "gid://shopify/ProductVariant/41891278487626", "quantity": 1}, 
        ],
        "customer": {
...
def fetch_products(base_url, limit=250):
    """
    Fetches products from the Shopify `/products.json` endpoint.
    Returns a dict containing the status, parsed products list (if any),
    and truncated raw text for debugging.
    """
    url = urllib.parse.urljoin(base_url, "/products.json")
    try:
        response = requests.get(url, params={"limit": limit}, timeout=DEFAULT_TIMEOUT)
        try:
            body = response.json()
        except Exception:
            body = {"text": response.text[:5000]}
        products = body.get("products") if isinstance(body, dict) else None
        count = len(products) if isinstance(products, list) else 0
        print(f"Fetched {count} products from {url}")
        return {"status": response.status_code, "products": products, "raw": body}
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--fetch-products", action="store_true", help="Fetch /products.json and save output")
    ...
    if args.buy_again:
        ...
        if order_to_buy_again:
            ...
            if items_to_buy and customer_for_order:
                ...
                result = attempt_buy(args.endpoint, items_to_buy, customer_for_order, is_buy_again=True, original_order_id=original_order_id)
            else:
    ...
    if result is None:
        if args.endpoint:
            ...
            result = attempt_buy(args.endpoint)
        else:
            ...
            for path, resp in discovery.items():
                if isinstance(resp, dict) and "mcp" in json.dumps(resp).lower():
                    ...
                    break
            if found_endpoint:
                result = attempt_buy(found_endpoint)
            else:
